			...
			Log.Logger = new LoggerConfiguration()
				...
				.WriteTo.Elasticsearch(
					new ElasticsearchSinkOptions(new Uri(settings.ApiUrl))
					{
						...
						IndexFormat = settings.IndexFormat.Empty() ? "logstash-{0:yyyy.MM.dd}" : settings.IndexFormat,
						ModifyConnectionSettings = x =>
							settings.UseBasicAuth ? x.BasicAuthentication(settings.Username, settings.Password) : x
					})
				.CreateLogger();
		}
	}
